...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', isLoading, children, ...props }, ref) => {
    const variants = {
      ...
      secondary: 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 shadow-sm focus:ring-gray-500',
...
export const Card: React.FC<{ className?: string; children?: React.ReactNode; onClick?: () => void }> = ({ className, children, onClick }) => (
  <div 
    onClick={onClick}
    className={cn('bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden', onClick && 'cursor-pointer hover:border-primary-300 transition-colors', className)}
  >
    {children}
  </div>
);
    ...
    return (
      <div className="w-full">
        <input
            ...
            {...props}
        />
        {error && <p className="mt-1.5 text-xs text-red-600 font-medium flex items-center gap-1 animate-in slide-in-from-top-1"><AlertTriangle className="w-3 h-3"/>{error}</p>}
      </div>
    );
  }
);
...
// Label Component
export const Label: React.FC<React.LabelHTMLAttributes<HTMLLabelElement> & { required?: boolean }> = ({ className, children, required, ...props }) => (
  <label className={cn('text-sm font-semibold text-gray-700 mb-1.5 block', className)} {...props}>
    {children}
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm animate-in fade-in duration-200">
            <Card className="w-full max-w-md shadow-2xl relative animate-in zoom-in-95">
                <CardContent className="p-8 text-center">
                    <div className={cn("w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm", variant === 'danger' ? 'bg-red-50 text-red-600' : 'bg-primary-50 text-primary-600')}>
                        <AlertTriangle className="w-8 h-8" />
                    </div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2">{title}</h3>
                    <p className="text-gray-500 mb-8 leading-relaxed">{description}</p>
                    <div className="flex gap-3 justify-center">
                        <Button variant="ghost" onClick={onClose} disabled={isLoading} className="w-24">
                            {cancelText}
                        </Button>
                        <Button variant={variant} onClick={onConfirm} isLoading={isLoading} className="w-32 shadow-md">
                            {confirmText}
                        </Button>
                    </div>
                </CardContent>
        ...
        <div className={className}>
            ...
            <div 
                className={cn(
                    ...
                    error ? "border-red-300 bg-red-50" : dragging ? "border-primary-500 bg-primary-50/50 scale-[1.02]" : "border-gray-300 hover:border-primary-400 hover:bg-gray-50",
            ...
            >
                ...
                {value ? (
                    <div className="relative w-full h-full flex flex-col items-center justify-center">
                        <img src={value} alt="Preview" className="max-h-[140px] rounded-lg object-contain mb-3 shadow-sm" />
                    ...
                    <div className="text-center space-y-3">
                        <div className={cn("w-12 h-12 rounded-full flex items-center justify-center mx-auto group-hover:scale-110 transition-transform shadow-sm", error ? "bg-red-100 text-red-500" : "bg-primary-50 text-primary-500")}>
                            {error ? <AlertTriangle className="w-6 h-6"/> : <Upload className="w-6 h-6" />}
                        </div>
                        <div>
                            {error ? (
                            ...
                            <p className="text-xs text-gray-400 mt-1 font-medium">{helperText}</p>
                        </div>
                    </div>
                )}
            </div>
