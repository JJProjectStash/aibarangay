          ...
          <div className="grid lg:grid-cols-3 gap-6">
             ...
             <div className="lg:col-span-2 grid md:grid-cols-2 gap-4">
                <button className="flex items-center justify-between p-4 bg-primary-600 text-white rounded-xl shadow-md hover:bg-primary-700 transition-colors group">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-primary-100 text-sm">Report issues in your area</p>
                    </div>
                    </div>
                ...
                <button className="flex items-center justify-between p-4 bg-blue-600 text-white rounded-xl shadow-md hover:bg-blue-700 transition-colors group">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-blue-100 text-sm">Borrow equipment or facilities</p>
                    </div>
                    </div>
                    <ArrowRight className="w-5 h-5 opacity-70 group-hover:translate-x-1 transition-transform" />
                </button>
             ...
             <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border border-yellow-100 rounded-xl p-5 shadow-sm">
                 <div className="flex items-center gap-2 mb-3">
                     ...
                     {latestAnnouncement?.isPinned && <Badge variant="danger" className="text-[10px]">PINNED</Badge>}
                 </div>
                 {loading ? <Skeleton className="w-full h-16" /> : latestAnnouncement ? (
                     <div>
                         <h4 className="font-semibold text-gray-900 line-clamp-1">{latestAnnouncement.title}</h4>
                         <p className="text-sm text-gray-600 mt-1 line-clamp-2">{latestAnnouncement.content}</p>
                         <p className="text-xs text-gray-400 mt-3">{new Date(latestAnnouncement.createdAt).toLocaleDateString()}</p>
                     </div>
                 ) : (
            ...
            <div className="h-[300px] w-full">
              {loading ? <Skeleton className="w-full h-full rounded-lg" /> : (
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    <Tooltip cursor={{ fill: '#f0fdfa' }} contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
                    ...
                    <div key={i} className="flex items-center gap-4 border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                        ...
                        <div>
                            <p className="text-sm font-medium text-gray-900">
                                {user.role === 'resident' ? `Your request #${1020+i} was updated` : `New complaint submitted by Resident #${100+i}`}
                            </p>
                            <p className="text-xs text-gray-500">{i * 2 + 5} mins ago</p>
                        </div>
                    </div>
                ))}
            </div>
